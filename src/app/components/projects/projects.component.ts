  ...
  template: `
    <section id="projects" class="py-32 relative">
      ...
      <div class="absolute inset-0 pointer-events-none opacity-30"
           style="background: radial-gradient(ellipse 80% 50% at 50% 100%, rgba(255,107,53,0.08), transparent)"></div>
      ...
      <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="reveal mb-8">
          <div class="rounded-3xl overflow-hidden border border-white/8 card-lift"
               style="background: linear-gradient(135deg, #1E2640 0%, #0E1220 100%)">
            <div class="grid lg:grid-cols-2">
              <!-- Visual side -->
              <div class="relative h-64 lg:h-auto min-h-72 overflow-hidden"
                   style="background: linear-gradient(135deg, rgba(255,107,53,0.2) 0%, rgba(0,212,255,0.1) 100%)">
                <div class="absolute inset-0 flex items-center justify-center">
                  <div class="text-center">
                    <div class="text-8xl font-display font-800 grad-text opacity-20">Rosr</div>
                    <div class="flex gap-3 mt-4 justify-center">
                      <span class="px-3 py-1 rounded-full text-xs bg-brand/20 border border-brand/30 text-brand font-body">E-Commerce</span>
                      <span class="px-3 py-1 rounded-full text-xs bg-electric/20 border border-electric/30 text-electric font-body">Admin</span>
                    </div>
                  </div>
                </div>
                <!-- Geometric decoration -->
                <div class="absolute -top-8 -right-8 w-32 h-32 rounded-full border border-brand/20 animate-spin-slow"></div>
                <div class="absolute -bottom-4 -left-4 w-24 h-24 rounded-full border border-electric/20 animate-spin-slow" style="animation-direction:reverse"></div>
                <span class="absolute top-4 left-4 px-2.5 py-1 rounded-full text-xs bg-lime/15 border border-lime/30 text-lime font-body font-600">⭐ Featured</span>
              </div>
              ...
              <div class="p-8 lg:p-10">
                ...
                <p class="font-body text-sm text-white/50 mb-6 leading-relaxed">
                  Conceived and implemented a scalable Nx Monorepo architecture with Module Federation,
                  partitioning the Storefront and Admin Dashboard into independently deployable applications
                  with clearly defined boundaries. Orchestrated complex state across 10+ NgRx slices.
                </p>

                <div class="flex flex-wrap gap-2 mb-8">
                  @for (tag of rosr.tags; track tag) {
                    <span class="px-2.5 py-1 rounded-lg text-xs font-body border border-white/10"
                ...
                <div class="flex gap-3">
                  <a href="https://github.com/Tawfykelgohre888/rosr" target="_blank"
                  ...
                  <div class="flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/10 text-white/50 text-sm font-body">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          @for (proj of otherProjects; track proj.title; let i = $index) {
            <div class="reveal card-lift rounded-2xl p-7 border border-white/8"
              ...
              <div class="flex items-start justify-between mb-4">
                ...
                <span class="font-body text-xs text-white/30">{{ proj.period }}</span>
              </div>
              <h3 class="font-display font-700 text-lg text-white mb-2">{{ proj.title }}</h3>
              <p class="font-body text-sm text-white/50 leading-relaxed mb-5">{{ proj.desc }}</p>
...
export class ProjectsComponent {
  ...
  rosr = {
    tags: ['Angular', 'NgRx', 'RxJS', 'Nx Monorepo', 'Module Federation', 'TypeScript', 'Tailwind CSS'],
  };
  ...
  otherProjects = [
    {
      ...
      github: 'https://github.com/Tawfykelgohre888/online-exam',
    },
    {
      ...
      github: 'https://github.com/Tawfykelgohre888/charity-app',
    },
  ];
}
