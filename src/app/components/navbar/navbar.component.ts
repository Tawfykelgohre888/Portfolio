...
@Component({
  ...
  template: `
    <nav
      ...
      [class]="
        scrolled()
          ? 'py-3 bg-dark-900/90 backdrop-blur-xl border-b border-white/5 shadow-2xl'
          : 'py-6'
      "
    >
      <div class="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <a href="#hero" class="font-display font-800 text-xl tracking-tight">
          <span class="grad-text">Tawfik</span
          ...
          @for (item of navItems; track item.label) {
            <li>
              <a
                ...
                [class]="
                  activeSection() === item.id ? 'text-brand' : 'text-white/60'
                "
              >
                {{ item.label }}
                <span
                  class="absolute -bottom-1 left-0 h-px bg-brand transition-all duration-300"
                  [class]="
        ...
        <div class="flex items-center gap-3">
          ...
          <a
            href="/Tawfik-Elgohare-CV-Final.pdf"
            download
          ...
          >
            <span
              class="w-5 h-px bg-white transition-all"
      ...
      @if (mobileOpen()) {
        <div
          class="md:hidden bg-dark-800/95 backdrop-blur-xl border-t border-white/5 px-6 py-4"
        >
          @for (item of navItems; track item.label) {
            <a
              [href]="item.href"
